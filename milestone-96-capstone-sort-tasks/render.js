...
function getVisibleTasks() {
  ...
  if (state.currentSort === SORTS.TITLE_ASC) {
    sortedTasks.sort((a, b) => a.title.localeCompare(b.title));
  }

  if (state.currentSort === SORTS.TITLE_DESC) {
    sortedTasks.sort((a, b) => b.title.localeCompare(a.title));
  }

  if (state.currentSort === SORTS.HIGH_PRIORITY_FIRST) {
    sortedTasks.sort(
      (a, b) => getPriorityRank(b.priority) - getPriorityRank(a.priority),
    );
  }

  if (state.currentSort === SORTS.LOW_PRIORITY_FIRST) {
    sortedTasks.sort(
